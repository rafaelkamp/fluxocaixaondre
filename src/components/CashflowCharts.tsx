...
export const CashflowCharts = ({ data }: CashflowChartsProps) => {
  ...
  const totalOutros = data.reduce((sum, item) => sum + (item.Outros || 0), 0);
  const netFlow = totalRevenue - totalExpenses + totalOutros;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="p-6 relative overflow-hidden" style={{ background: "var(--gradient-card)" }}>
          <div
            className="absolute top-0 right-0 w-32 h-32 opacity-10"
            style={{ 
              background: totalOutros >= 0 
                ? "var(--gradient-secondary)" 
                : "linear-gradient(135deg, hsl(0 84.2% 60.2%) 0%, hsl(0 62.8% 50%) 100%)" 
            }}
          />
          <div className="relative space-y-2">
            <div className="flex items-center gap-2 text-muted-foreground">
              <DollarSign className="w-4 h-4" />
              <span className="text-sm font-medium">Outros (Tesouraria)</span>
            </div>
            <p className={`text-3xl font-bold ${totalOutros >= 0 ? "text-secondary" : "text-destructive"}`}>
      ...
      <Card className="p-6" style={{ background: "var(--gradient-card)" }}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data}>
            ...
            <Bar dataKey="Outros" fill="hsl(var(--accent))" radius={[8, 8, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
    </div>
